...
namespace LiteFx.Web.Mvc
{
	...
	public abstract class ContentType
	{
		...
		public static ContentType Tiff { get { return new TiffContentType(); } }
		...
		public static ContentType FromString(string extension)
		{
			try
			{
				Format format = (Format)Enum.Parse(typeof(Format), extension, true);
				return FromFormat(format);
			}
			catch (Exception ex)
			{
				throw new InvalidCastException(string.Format("Não foi possível converter a extensão '{0}'", extension), ex);
			}
		}
	...
	public sealed class TiffContentType : ContentType
	{
		public override string ContentTypeString
		{
